MODULE usrdef_zgr
   ...
   USE usrdef_nam, ONLY: rn_domszz, nn_botcase, rn_maxz
...
CONTAINS             
   ...
   SUBROUTINE zgr_z( pdept_1d, pdepw_1d, pe3t_1d , pe3w_1d )   ! 1D reference vertical coordinate
      ...
      REAL(wp) ::   ztanh_max, ztanh_scale, ztanh_val, z_tanh_z, z_tanh_z_norm 
      !!----------------------------------------------------------------------
      !
      ...
      IF(rn_maxz==0.) THEN
         ...
         END DO
      ELSE
         ztanh_max = (jpk - 0.8_wp*jpk)/200
         ztanh_scale = (jpk-1) + (1./2. + 1./2.*TANH(2*rpi*ztanh_max))*(rn_maxz-(jpk-1))/(jpk-1)*(jpk-1)
         !CALL z_tanh_max(ztanh_scale) 
         DO jk = 2, jpk          ! depth at T and W-points
            ztanh_val = (( jk ) - 0.8*jpk )/200
            z_tanh_z  = (jk-1) + (1./2.+1./2.*TANH(2*rpi*ztanh_val)) *(rn_maxz-(jpk-1))/(jpk-1)*(jk-1) 
            z_tanh_z_norm = ( z_tanh_z / ztanh_scale ) * rn_maxz
            pdepw_1d(jk) = z_tanh_z_norm
            pdept_1d(jk) = z_tanh_z_norm + (( z_tanh_z_norm - pdepw_1d(jk-1))/ 2 )
            !CALL z_tanh( jk, ztanh_scale, pdept_1d, pdepw_1d)
         END DO
      ENDIF
      !
