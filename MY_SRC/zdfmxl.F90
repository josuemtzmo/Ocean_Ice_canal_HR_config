...
!CT SEDNA {
      INTEGER , DIMENSION(jpi,jpj) ::  zsednmln
      REAL(wp), DIMENSION(jpi,jpj) ::  zsedhmlp
!CT SEDNA }
      !!----------------------------------------------------------------------
      !
      ...
      !
!CT for SEDNA { 
!CT Perfrom a different online diagnostic

      ! w-level of the mixing and mixed layers
      DO_2D_OVR( nn_hls, nn_hls, nn_hls, nn_hls )
         zsednmln(ji,jj)  = 2               ! Initialization to the number of w ocean point
         zsedhmlp(ji,jj)  = 0._wp               ! here hmlp used as a dummy variable, integrating vertically N^2
      END_2D
      DO_3D_OVR( nn_hls, nn_hls, nn_hls, nn_hls, 2, jpkm1 )
         ikt = mbkt(ji,jj)
         zsedhmlp(ji,jj) = zsedhmlp(ji,jj) + MAX( rn2b(ji,jj,jk) , 0._wp ) * e3w(ji,jj,jk,Kmm)
         IF( zsedhmlp(ji,jj) < zN2_c )   zsednmln(ji,jj) = MIN( jk , ikt ) + 1   ! Mixed layer level
      END_3D
      !
      DO_2D_OVR( nn_hls, nn_hls, nn_hls, nn_hls )
         iik = zsednmln(ji,jj)
         zsedhmlp (ji,jj) = gdepw(ji,jj,iik,Kmm) * ssmask(ji,jj)    ! Mixed layer depth
      END_2D

      IF( .NOT.l_offline .AND. iom_use("samldr10_1") ) THEN
         CALL iom_put( "samldr10_1", zsedhmlp )            ! mixed layer depth
      ENDIF
!CT for SEDNA }
      !
...
!CT SEDNA {
      INTEGER, DIMENSION(A2D(nn_hls)) ::   zsedimld
      REAL(wp), DIMENSION(jpi,jpj) ::   zsedhmld
!CT SEDNA }
      !!----------------------------------------------------------------------
      !
...
!CT for SEDNA {
      ! w-level of the turbocline and mixing layer (iom_use)
      zsedimld(:,:) = mbkt(A2D(nn_hls)) + 1                ! Initialization to the number of w ocean point
      DO_3DS( 1, 1, 1, 1, jpkm1, 2, -1 )   ! from the bottom to nlb10
               IF( avt (ji,jj,jk) < avt_c * wmask(ji,jj,jk) )   zsedimld(ji,jj) = jk      ! Turbocline 
      END_3D
      ! depth of the mixing and mixed layers
      DO_2D_OVR( 1, 1, 1, 1 )
         iik = zsedimld(ji,jj)
         zsedhmld (ji,jj) = gdepw(ji,jj,iik,Kmm) * ssmask(ji,jj)    ! Turbocline depth 
      END_2D

      IF( .NOT.l_offline .AND. iom_use("samldkz5") ) THEN
         CALL iom_put( "samldkz5"  , zsedhmld )             ! turbocline depth
      ENDIF
!CT for SEDNA }
